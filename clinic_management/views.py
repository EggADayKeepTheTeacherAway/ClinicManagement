    ...
    def get_context_data(self, **kwargs):
        ...
        for appointment in appointments:
            ...
            medical_records_for_patient = [
                record for record in medical_records if record.PatientID_id == patient.PatientID
            ]
            ...
            for medical_record in medical_records_for_patient:
                ...
                disease = disease_mapping.get(medical_record.DiseaseID_id, None)
                payment = payment_mapping.get(medical_record.MedicalRecordID, None)

                # Treatment for the specific medical record
                treatment = treatment_mapping.get(disease.TreatmentID_id) if disease else None

                # Medicine details and dosage
                medicine_records_for_appointment = [
                    record for record in medicine_records if record.MedicalRecordID_id == medical_record.MedicalRecordID
                ]
                ...
                for medicine_record in medicine_records_for_appointment:
                    dosage = dosage_mapping.get(medicine_record.DosageID_id, None)
